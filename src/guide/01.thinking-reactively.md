...
简单来说，分离同步代码和异步代码运行时的主要因素是延迟，也就是等待时间。明确的编码时间是很难确定的。当你能够理解与自己的书写顺序一致的同步执行时，就更容易推倒出解决方案：“执行这段程序，然后立即执行另一段程序”。

但是计算机世界并没有提供如此奢侈的操作. 在这个高度网络化计算的时代，发送消息和接受响应所花费的时间，代表了应用能够处理一些其他事情的临界时间，例如响应用户输入，处理数字，或者更新UI. 这更像是“做这个（等待一段不确定的时间）；然后做另一个。”使应用程序闲置等待数据库的查询返回，网络的响应，或者是用户操作的完成，这些传统方法是不可接受的，因此需要你利用异步操作，以使应用程序始终处于响应状态。这里的主要问题是是否可以在长时间运行的进程中阻塞用户。
