...
使用函数作为回调已经成为javascript开发的主要部分，这种情况已经很多年了。回调用于包括从鼠标点击和键盘操作到处理远端http请求或者文件I/O的一切事情. JavaScript,作为一种单线程语言，需要这样一种结构才能保证所有级别的可用性. 创建回调函数用来解决由于长时间运行操作所导致的阻塞问题，通过允许你提供一个处理函数，让javascript运行时在数据准备就绪后就调用。同时，你的应用程序能够继续执行其他的任务，如图1.2所示。

在图1.2中，回调函数在javascript中创建了一种控制反转，其中是函数调用应用程序，而不是相反。

与以往的代码不同，你不必等待阻塞的HTTP调用，使用异步（AJAX）请求的回调创建一个控制反转，允许你的应用程序继续执行下一行代码. 在这种意义下，控制反转指的是这样一种方式，你的代码的某些部分接收来自运行时系统（runtime）的控制流. 在这种情况下，当数据准备好可以被处理的时候，运行时通过函数处理程序调用（或者返回指令给你），由此，就是术语callback。来看这段代码:
...
回调函数允许你异步执行代码，因此应用程序可以稍后返回控制指令给你. 在这个代码示例中，HTTP请求在后台运行，并将控制指令立即返回给调用者以便开始渲染UI；它只有在完全加载后才处理项目的内容. 这种行为是理想的，因为它释放了应用程序，以便在处理其他任务（如加载网页的其余部分）方面取得进展，如本例所示. 正如你将在本书中看到的，异步代码是一种很好的设计，适用于处理I／O绑定的任务，如从web或者数据库获取数据. 这样做的原因是I／O进程通常比任何其他类型的指令慢得多，所以我们允许它们在后台运行，因为它们的完成不依赖处理周期.

> 在1.1.2节的代码示例中，ajax（）的第二个参数是回调函数. 在该代码中，如本书的许多部分，我们使用ECMASript 6中的lambda表达式语法，它提供了一种更简单和更简洁的函数调用的方法. lambda表达式也被称为箭头函数，lambda表达式和我们早已熟悉的匿名函数调用类似. 箭头函数和匿名函数的微小差别在于它的关键字this的指向。在极少数情况下，当this的值很重要时，我们将会把它提出来，并且改用使用匿名表达式。
...
事件发射器是用于基于异步事件的体系结构的流行机制. 例如，DOM就是最广为人知的事件发射器之一. 在像node.js这样的服务器上，某些类型的对象会周期性的产生事件来促使函数调用. 在node.js中，EventEmitter类用于实现诸如websocket I/O或者文件读／写之类的API，因此如果你遍历目录要找到目标文件，事件对象会发出引用此文件的事件供你执行后续任何额外的代码.

让我们实现一个简单的对象来稍微展示一下这个API. 考虑一个简单的计算器对象，可以发出像加法和减法这样的事件，你可以将任何自定义逻辑挂载（hook），如图1.6.

图1.6节点发射器，表示一个简单的计算器，它暴露了两个事件-加法和减法：
...
订阅事件发射器是通过addListener()完成的，它允许你提供一个回调函数，以便于在目标事件触发的时候调用。 不幸的是，事件发射器跟使用回调处来处理来自多个组合资源的数据的相比，也具有所有相同的问题。总体而言，组合嵌套异步流是很困难的.

整个JavaScript社区在解决这些问题方面取得了长足进展. 借助于FP中出现的模式，你可以使用ES6提供的替代方案，也就是promise.
